...
export async function PATCH(req, { params }) {
  ...
  if (label && label.trim() !== type.label) {
    await db.collection("maintenance_records").updateMany({ type_key: key }, { $set: { type_label: label.trim() } });
  }
...
export async function DELETE(req, { params }) {
  ...
  const { key } = params;
  await db.collection("maintenance_records").deleteMany({ type_key: key });
  await db.collection("maintenance_types").deleteOne({ _id: key });
  return NextResponse.json({ ok: true });
}

...
export default function SaatGecmisiPage() {
  ...
  const dailyAverage = useMemo(() => {
    ...
    for (const entry of history) {
      ...
      lastEntryByDay.set(dayKey, entry);
    }

    const dailyEntries = [...lastEntryByDay.values()].sort((a, b) => new Date(a.date) - new Date(b.date));
    if (dailyEntries.length < 2) return 0;

    const deltas = dailyEntries.slice(1).map((entry, index) => entry.hours - dailyEntries[index].hours);
    const average = deltas.reduce((sum, delta) => sum + delta, 0) / deltas.length;
    return Math.min(24, average);
  }, [history]);
          ...
          <>
            <div className="grid grid-cols-3 gap-2 mb-4">
              ...
              <div className="bg-panel border border-border rounded-card p-2.5">
                ...
                <div className="font-mono text-[15px] font-bold text-amber mt-1">{dailyAverage.toFixed(1)} sa</div>
              </div>
              <div className="bg-panel border border-border rounded-card p-2.5">

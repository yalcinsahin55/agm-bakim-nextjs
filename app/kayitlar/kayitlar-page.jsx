...
export default function KayitlarPage() {
  ...
  return (
    <div>
                ...
                <div key={r._id} className="bg-panel border border-border rounded-card p-3.5">
                  {(photos.length > 0 || videos.length > 0) && (
                    <div className="flex gap-1.5 flex-wrap mb-2">
                      {photos.map((p, idx) => (
                        <img key={`p${idx}`} src={`data:image/jpeg;base64,${p}`} className="w-14 h-14 rounded-lg object-cover border border-border" alt="" />
                      ))}
                      {videos.map((v, idx) => (
                        <button key={`v${idx}`} onClick={() => setVideoModal(v)} className="relative w-14 h-14 rounded-lg overflow-hidden border border-border flex-shrink-0">
                          <video src={`data:${v.mime};base64,${v.data_b64}`} className="w-full h-full object-cover" preload="metadata" muted />
                          <span className="absolute inset-0 flex items-center justify-center bg-black/35 text-white text-base">▶</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {r.grouped_with && (
                    <div className="text-[10.5px] text-teal mt-1">🔗 {r.grouped_with} ile birlikte tamamlandı — fotoğraf/not o kayıtta</div>
                  )}
      ...
      {videoModal && (
        <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4" onClick={() => setVideoModal(null)}>
          <video
            src={`data:${videoModal.mime};base64,${videoModal.data_b64}`}
            controls autoPlay className="max-w-full max-h-full rounded-xl"
            onClick={(e) => e.stopPropagation()}
          />
          <button onClick={() => setVideoModal(null)} className="absolute top-4 right-4 w-9 h-9 rounded-full bg-panel2 border border-border text-text text-lg">✕</button>
        </div>
      )}

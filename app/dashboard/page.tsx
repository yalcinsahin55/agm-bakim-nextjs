...
function DashboardAssistant(): JSX.Element {
  ...
  return <div className="mb-4 rounded-card border border-teal/30 bg-gradient-to-br from-teal/10 via-panel to-panel p-4 animate-fade-in">
    ...
    <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
      {DASHBOARD_ASSISTANT_QUICK_QUESTIONS.map((item) => <button key={item} type="button" onClick={() => void ask(item)} disabled={sending} className="flex-shrink-0 rounded-full border border-border bg-panel2 px-2.5 py-1.5 text-[9.5px] font-semibold text-muted transition hover:border-teal/50 hover:text-text disabled:cursor-not-allowed disabled:opacity-50">{item}</button>)}
    </div>

  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm md:items-center md:p-4" role="dialog" aria-modal="true" aria-label="Bakım kaydı detayı">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl border border-border bg-panel p-4 shadow-2xl animate-fade-in md:rounded-2xl">
        ...
        {record.component_transfers?.length ? <div className="mt-2 rounded-lg border border-cyan-300/30 bg-cyan-300/5 p-3 text-[11px] text-cyan-100"><b>Değişen parça saatleri:</b><div className="mt-2 flex flex-col gap-2">{record.component_transfers.map((transfer) => <div key={transfer.id} className="rounded-lg border border-border bg-panel2 p-2"><div className="font-bold text-text">{transfer.component_name} · {transfer.condition === "new" ? "Yeni" : "Çıkma"}</div><div className="mt-0.5 text-[10px] text-muted">Önceki toplam: <b>{transfer.source_hours.toLocaleString("tr-TR")} sa</b> · Takıldığı motor saati: <b>{transfer.installed_hours.toLocaleString("tr-TR")} sa</b> · Kayıt anındaki toplam: <b>{transfer.source_hours.toLocaleString("tr-TR")} sa</b></div><div className="mt-0.5 text-[10px] text-cyan-200">Hesap formülü: önceki parça saati + ({transfer.installed_hours.toLocaleString("tr-TR")} − takılma saati) · Güncel motor saati geldikçe toplam otomatik ilerler.</div>{transfer.note && <div className="mt-1 text-[10px] text-faint">{transfer.note}</div>}</div>)}</div></div> : null}

    ...
    <div className="bg-panel border border-border rounded-card p-3.5 hover:border-borderlt transition-all">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        ...
        {canEdit && <>
          ...
          {deletePending ? <><button type="button" onClick={onDeleteConfirm} className="rounded-lg bg-red px-2.5 py-1.5 text-[11px] font-bold text-[#1a1206] hover:brightness-110 transition">Evet, Sil</button><button type="button" onClick={onDeleteCancel} className="rounded-lg border border-border px-2.5 py-1.5 text-[11px] font-bold text-muted hover:bg-panel2 transition">Vazgeç</button></> : <button type="button" onClick={onDeleteRequest} className="rounded-lg border border-red/40 px-2.5 py-1.5 text-[11px] font-bold text-red hover:bg-red/10 transition">🗑️ Sil</button>}
        </>}
      </div>

...
test("backup and upload large-payload paths use bounded processing", async () => {
  ...
  const audit = await source("lib/audit.ts");
  ...
  assert.match(performance, /AsyncLocalStorage/);
  assert.match(performance, /getCurrentRequestId/);
  assert.match(performance, /X-Request-Id/);
...
test("maintenance report attachments stay bounded, authenticated, and offline-safe", async () => {
  ...
  assert.match(uploadHelper, /uploadReportAttachmentChunked/);
  assert.match(uploadHelper, /chunkedUpload: true/);
  assert.match(chunkUpload, /uploadReportAttachmentChunked/);
...
test("service worker updates bypass stale script caches", async () => {
  ...
  assert.match(serviceWorker, /agm-bakim-shell-v7/);

...
const APPLY_BRANCH = "migration-remainder-apply-b";
const REPORT_PATH = "/tmp/agm-legacy-media-remainder-b-report.json";
const BACKUP_PATH = "/tmp/agm-legacy-media-remainder-b-backup.json";
...
function main(): void {
  ...
  const runId = `legacy-media-remainder-b-${process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 12) || "unknown"}`;
  const args = [

...
const VERIFY_BRANCH = "migration-oil-pdf-batch-b-verify";
const RUN_ID = "legacy-oil-pdf-batch-b-dpl_4mA4cg7z2DAU9QhyzxEJaEmD1YHr";
const REPORT_PATH = "/tmp/agm-legacy-oil-pdf-batch-b-verify-dry-run.json";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const ok = Boolean(run && run.status === "completed" && numeric(run.applied) === 6 && numeric(run.pending) === 0 && numeric(run.errors) === 0 && items.length === 6 && blobReachable === 6 && referencesReady === 6 && dryRun && numeric(dryRun.scanned) === 0 && numeric(dryRun.eligible) === 0 && numeric(dryRun.invalid) === 0 && numeric(dryRun.total_bytes) === 0 && dryRun.limited !== true);
    console.log(JSON.stringify({ migration: "legacy-oil-pdfs", mode: "verify", ok, run_status: run?.status || "missing", applied: numeric(run?.applied), pending: numeric(run?.pending), errors: numeric(run?.errors), backup_items: items.length, blob_reachable: blobReachable, references_ready: referencesReady, final_scanned: numeric(dryRun?.scanned), final_eligible: numeric(dryRun?.eligible), final_invalid: numeric(dryRun?.invalid), final_total_bytes: numeric(dryRun?.total_bytes), final_limited: dryRun?.limited === true }));

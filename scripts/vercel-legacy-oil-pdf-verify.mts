...
const VERIFY_BRANCH = "migration-oil-pdf-pilot-verify";
const RUN_ID = "legacy-oil-pdf-pilot-dpl_Dkq6V4T7bsvEmkD8Kf7TuJfuCnNG";
const REPORT_PATH = "/tmp/agm-legacy-oil-pdf-verify-dry-run.json";

function numeric(value: unknown): number {
  return typeof value === "number" && Number.isFinite(value) ? value : 0;
}
function idFilter(value: string): ObjectId | string {
...
function runDryRun(): NumericReport | null {
  ...
  const child = spawnSync(
    ...
    { env: process.env, encoding: "utf8", stdio: ["ignore", "ignore", "ignore"] },
  );
  if (child.status !== 0 || !existsSync(REPORT_PATH)) return null;
  try {
    return JSON.parse(readFileSync(REPORT_PATH, "utf8")) as NumericReport;
  } catch {
    return null;
  }
}
async function main(): Promise<void> {
  ...
  if (!uri) {
    console.error(JSON.stringify({ migration: "legacy-oil-pdfs", mode: "verify", ok: false, reason: "MONGO_URI missing" }));
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    const documents = oilIds.length > 0
      ? await db.collection<OilDocument>("oil_analyses").find({ _id: { $in: oilIds.map(idFilter) } }).project({ _id: 1, pdf_url: 1, pdf_b64: 1 }).toArray()
      : [];
    let blobReachable = 0;
    for (const item of items) {
      ...
      for (const url of urls) {
        try {
          const result = await get(url, { access: "private", ...blobCredentials(), abortSignal: AbortSignal.timeout(20_000) });
          if (result?.statusCode === 200) blobReachable += 1;
        } catch {
          // Aggregate failure is reported without exposing URL or error details.
        }
      }
    }
    ...
    const ok = Boolean(run && run.status === "completed" && numeric(run.applied) === 3 && numeric(run.pending) === 0 && numeric(run.errors) === 0 && items.length === 3 && blobReachable === 3 && referencesReady === 3 && dryRun && numeric(dryRun.eligible) === 12 && numeric(dryRun.invalid) === 0 && dryRun.limited !== true);
    console.log(JSON.stringify({
      migration: "legacy-oil-pdfs",
      mode: "verify",
      ok,
      run_status: run?.status || "missing",
      applied: numeric(run?.applied),
      pending: numeric(run?.pending),
      errors: numeric(run?.errors),
      backup_items: items.length,
      blob_reachable: blobReachable,
      references_ready: referencesReady,
      remaining_scanned: numeric(dryRun?.scanned),
      remaining_eligible: numeric(dryRun?.eligible),
      remaining_invalid: numeric(dryRun?.invalid),
      remaining_total_bytes: numeric(dryRun?.total_bytes),
      remaining_limited: dryRun?.limited === true,
    }));
...
main().catch(() => {
  console.error(JSON.stringify({ migration: "legacy-oil-pdfs", mode: "verify", ok: false, reason: "verification failed" }));
  process.exitCode = 1;
});

...
async function migrateRecord(record: MigrationRecord, db: Db, onBeforeCommit: (change: PendingChange) => Promise<void>): Promise<PendingChange | null> {
  ...
  try {
    ...
    for (const [index, value] of photos.entries()) {
      ...
      const blob = await put(legacyBlobPath(record._id, "photo", index, parsed.buffer, extension), parsed.buffer, { access: "private", contentType: parsed.mime, addRandomSuffix: false, allowOverwrite: true, ...(token ? { token } : {}), ...(blobStoreId() ? { storeId: blobStoreId() } : {}) });
    ...
    for (const [index, value] of videos.entries()) {
      ...
      const blob = await put(legacyBlobPath(record._id, "video", index, parsed.buffer, videoExtension), parsed.buffer, { access: "private", contentType: parsed.mime, multipart: true, addRandomSuffix: false, allowOverwrite: true, ...(token ? { token } : {}), ...(blobStoreId() ? { storeId: blobStoreId() } : {}) });

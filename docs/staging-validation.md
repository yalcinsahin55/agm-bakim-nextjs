...
Staging’de önce yönetici hesabıyla backup export alınır ve JSON dosyası güvenli, lokal test klasöründe tutulur. Export içinde `password`, `password_hash`, token, VAPID secret ve legacy base64 dosya alanlarının bulunmadığı doğrulanır. Restore endpoint’ine `confirm: "RESTORE"` ile gerçek yazma göndermeden önce aynı dosya `dry_run: true` ile test edilir; `summary`, `skipped`, `mode: "dry-run"` ve `applied: false` kontrol edilir.

Round-trip testi için staging snapshot’ı alınmalı, test verisiyle kontrollü değişiklik yapılmalı, restore sonrası motorlar, bakım türleri, bakım kayıtları ve yağ analizleri sayıları karşılaştırılmalıdır. Restore route’u yalnızca allowlist’teki dört koleksiyona merge yazar; kullanıcılar, notification’lar ve audit log backup içinde olsa da restore edilmez. Bu bilinçli güvenlik kararıdır. Production restore kesinlikle bu runbook’tan otomatik çalıştırılmamalıdır.
